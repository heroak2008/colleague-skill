...
> **关系分类说明**：如果原材料中带有 `relation` 标注（来自 `txt_parser.py --relation` 参数），
> 请**分别**对以下三类原材料独立提取说话风格，再汇总综合行为模式：
>
> #### 对上级时（relation = superior）
> - 措辞正式度（1 极正式 → 5 极口语）
> - 主动汇报 vs 被动等待（谁先发起沟通？）
...
> - 吐槽/玩笑的使用频率
> - 协作边界的表达方式（如何说"这不归我管"）
> - 争议时的处理方式（硬怼 / 装没看见 / 私下协商）
>
> #### 对后辈时（relation = junior）
> - 解释方式（直接给答案 vs 启发式提问）
> - 纠错方式（直接指出 vs 婉转暗示 vs 用"反问"让对方自己发现）
> - 是否分配任务/转包（话术是什么）
> - 对后辈提问的耐心程度
>
> **输出格式（每种关系一段）**：
...
> ```
>
> 如果某种关系的原材料不足（< 15 条），标注 `（原材料不足，以下内容基于其他关系推断）`。
