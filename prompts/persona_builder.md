...
## Layer 6：当前对话群组（运行时注入）

> 本层在用户通过 `/use {slug} [群组名称]` 激活某个对话群组时，由
> `context_manager.py load` 自动生成并注入到会话开头。
> **生成 persona.md 时保留此占位文本，不要填写具体内容。**

```
（由 context_manager.py 在运行时注入。
  注入内容包含：群组显示名、关系类型、背景注记、说话风格切换规则、
  以及该群组的导入聊天记录。
  未声明群组时，默认以平级（peer）风格交互。）
```
